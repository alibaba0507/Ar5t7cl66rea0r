package articlecreator.spin.syntax;
...
import articlecreator.spin.syntax.net.HTTPConnection;
...
public class SynonymFinder {
    ...
    public SynonymFinder(boolean searchOnline) throws Exception {
        this.searchOnline = searchOnline;
        hashMapOfFrequency = new HashMapOfFrequency();
        hashMapOfSynonyms = new HashMapOfSynonyms();
        // File f = new File(ArticleManagmentMain.class.getResource("/resourses/freq.txt").toString());
        ...
        File f = new File("syn.txt");
        if (f.exists() == false) { // copy file out of jar if not there

            AWTUtils.exportResource("/resourses/syn.txt");
        }
        loadSynonyms("syn.txt");
    ...
    private String findSynonym(String word) throws Exception {
        ...
        if (answerPriority <= 5000) {
            ...
            upWrite("syn.txt", word + "-" + answer);
            return word;
        }
        if (searchOnline) {
            answer = getOnlineSearch(word, answerPriority);
        } else {
            answer = getLocalSearch(word);
        }
        if (answer != null) {
    ...
    private String getLocalSearch(String word) throws Exception {
        String answer = null;
        int count = 0;
        // FileReader fileIn = new FileReader(file);
        InputStream in = getClass().getResourceAsStream("/synonims/synonyms_1.txt");
    ...
    private void loadSynonyms(String fileName) throws IOException {
        ...
        if (amountOfBadWords >= MAX_AMOUNT_OF_BAD_WORDS) { // the most adequate constant
            file.delete();
            PrintWriter localPrintWriter = new PrintWriter(new OutputStreamWriter(new FileOutputStream("syn.txt")));
