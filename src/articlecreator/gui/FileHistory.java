...
public class FileHistory {
   ...
   *******************************************************************/
      ...
      if (new File(historySerFile).exists()) {
         try {
            ...
            for (int i=0; i<itemnameCount; i++) {
               ...
               String pathname = (String)ois.readObject();
               File file = new File(pathname);
               fileList.add(file);
               // skip non existing file history entries
               if (file.exists()) {
                   itemnameHistory.add(itemname);
                   pathnameHistory.add(pathname);
                   MenuItemWithFixedTooltip item = new MenuItemWithFixedTooltip((itemNbr+1)+": "
                                                      +(String)itemnameHistory.get(itemNbr));
                   item.setToolTipText((String)pathnameHistory.get(itemNbr));
                   item.addActionListener(new ItemListener(itemNbr));
                   fileMenu.add(item);
                   itemNbr++;
               }
               else {
                   System.out.println("History file does not exist: "+file.getAbsolutePath());
               }
            }
            ois.close();
            fis.close();         
   ...
   /**
    * Return an ArrayList of File objects, each representing an entry
    * in the current list of files that can be opened via our
    * file menu add on.
    */
   ...
   *******************************************************************/
   public final void insertPathname(String pathname) {
      ...
      if (itemnameHistory.size() == max_itemnames) {
         // file list is full: remove last entry to get space for the first item
         ...
         public void actionPerformed(ActionEvent e) {
            ...
            if (indicesToDelete.length > 0) {
               ...
               for (int i=0; i<itemnameHistory.size(); i++) {
                  MenuItemWithFixedTooltip item = new MenuItemWithFixedTooltip((i+1)+": "
                     +(String)itemnameHistory.get(i));
                  String pathname = (String)pathnameHistory.get(i);
